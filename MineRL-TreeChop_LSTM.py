...
class ActorCritic(tf.keras.Model):
  ...
  def call(self, inputs: tf.Tensor, memory_state: tf.Tensor, carry_state: tf.Tensor, training) -> Tuple[tf.Tensor, tf.Tensor, 
    ...
    #conv_3_flattened = layers.Flatten()(conv_3)
    
    initial_state = (memory_state, carry_state)
    #print("initial_state: ", initial_state)
    lstm_output, lstm_state = self.lstm(conv_3_reshaped, initial_state, training=training)
...
tree_data = minerl.data.make('MineRLTreechop-v0', data_dir="/home/kimbring2/minerl_data")
